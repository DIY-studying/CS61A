...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        # BEGIN PROBLEM EC
        "*** YOUR CODE HERE ***"
        typed_tuple=(item for item in typed)
        source_tuple=(item for item in source)
        pair_tuple=(typed_tuple,source_tuple)
        if pair_tuple not in cache or cache[pair_tuple][1]<limit:
            result=diff_function(typed,source,limit)
            cache[pair_tuple]=[result,limit]
            return result
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    "*** YOUR CODE HERE ***"
    def diff_num(source):
        return diff_function(typed_word,source,limit)
    smallest_num=min(word_list,key=diff_num)
    if diff_num(smallest_num)>=limit:
        return typed_word
    return smallest_num
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    return furry_fixes(typed[0:1],source[0:1],limit)+furry_fixes(typed[1:],source[1:],limit)
    # END PROBLEM 6
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(0,len(timestamps_per_player)):
        player_i=timestamps_per_player[i]
        timer_i=[]
        for j in range(0,len(player_i)-1):
            timer_i+=[player_i[j+1]-player_i[j]]
        times+=[timer_i]
        timer_i=[]
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for j in word_indices:
        min_player=0
        for i in player_indices:
            if get_time(times,min_player,j)>get_time(times,i,j):
                min_player=i
        fast_word[min_player]+=[words[j]]
    return fast_word
    # END PROBLEM 10
