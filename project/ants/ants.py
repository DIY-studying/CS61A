...
class ThrowerAnt(Ant):
    ...
    def place_gen(self):
        ...
        for _ in range(self.lower_bound):
            ...
            if current_place.is_hive:
                break
        while index<=self.upper_bound and not current_place.is_hive:
            yield current_place
            current_place=current_place.entrance
            index+=1
        yield None
    ...
    def nearest_bee(self):
        ...
        gen=self.place_gen()
        current_place=next(gen)
        while current_place:
            near_bee=random_bee(current_place.bees)
            if near_bee:
                return near_bee
            current_place=next(gen)
        return None
    ...
    def throw_at(self, target):
        ...
        def slow_action(self,gamestate):
            ...
            if current_turn==max_turn:
                self.action=MethodType(action, target )
        target.action=MethodType(slow_action, target )
        # END Problem EC 1
...
class ScaryThrower(ThrowerAnt):
    ...
    # BEGIN Problem EC 2
    implemented = True   # Change to True to view in the GUI
    # END Problem EC 2python3 ok -q EC1 -u
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem EC 4
        # print("DEBUG",self.damage*((1-1/16)**self.insects_shot))
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.is_scare and not self.scare_turn==self.scare_length:
