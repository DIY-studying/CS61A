...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls>0:
        ...
        if(num_die==1):
            has_1=True
        total,num_rolls=total+num_die,num_rolls-1
    if(has_1):
        total=1
    return total
    # END PROBLEM 1
def digit_number(num,digit):
    """Return the digit of a number.

    num:    The number.
    digit:  the digit you want return.
    ...
    """
    assert type(num)==int or type(digit)==int,"must be an integer."
    assert digit>0, "must great zero."
    last_num=num
    while digit>1:
        last_num=num//10
        digit-=1
...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    player_one,opponent_two=digit_number(player_score,1),digit_number(opponent_score,2)
    tri_abs=3*abs(player_one-opponent_two)
    if(tri_abs<1):
        return 1
    return tri_abs
    # END PROBLEM 2
...
def sus_points(score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    if(not (num_factors(score)==4 or num_factors(score)==3)):
        return score
...
def play(strategy0, strategy1, update, score0=0, score1=0, dice=six_sided, goal=GOAL):
    ...
    while score0<goal and score1<goal:
        if(who==0):
            player0_roll=strategy0(score0,score1)
            score0=update(player0_roll,score0,score1,dice)
        else:
            player1_roll=strategy1(score1,score0)
            score1=update(player1_roll,score1,score0,dice)
        who=1-who
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    i,num_rolls,max_num=1,1,0
    while i<=10:
        average_num=average_dice(i,dice)
        if(average_num>max_num):
            max_num=average_num
            num_rolls=i
        i+=1
    return num_rolls
